...
export default function DragDrop() {
  ...
  return (
    <div className="container">
      <div
        {...getRootProps()}
        className={`group flex w-full flex-col items-center justify-center rounded-md border-2 border-dashed border-slate-300 py-3 text-sm font-medium leading-6 text-slate-900 hover:border-solid hover:border-blue-500 hover:bg-white hover:text-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:border-slate-600 dark:hover:bg-slate-700/40 dark:hover:text-slate-300
        ${
          isDragActive
      ...
      >
        <input {...getInputProps()} />
        {isDragActive ? (
          <p className="font-medium text-blue-600">ปล่อยไฟล์ที่นี่...</p>
        ) : (
          <p className="text-gray-600">
            ลากไฟล์มาวาง .json/.yaml/.yml หรือคลิกเพื่อเลือกไฟล์
          </p>
        )}
        <FileDown className="mx-auto mb-1 size-12 text-slate-400 drop-shadow-md group-hover:text-blue-500 dark:group-hover:text-slate-300 " />
      </div>
      {files.length > 0 && (
        <aside>
          <h4>Files</h4>
