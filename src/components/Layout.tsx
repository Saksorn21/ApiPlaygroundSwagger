...
const Layout: React.FC<LayoutProps> = ({
  ...
  return (
    <ThemedBox>
      <div className="flex min-h-screen flex-col bg-white text-slate-500 transition-colors dark:bg-slate-900 dark:text-slate-400">
        <header className="sticky top-0 z-40 w-full bg-white/30 backdrop-blur-md transition-colors duration-500 dark:border-slate-50/[0.06] dark:bg-slate-900/75 lg:z-50 lg:border-b lg:border-slate-900/10">
          <div className="max-w-8xl mx-auto">
            <div className="mx-4 flex h-16 items-center justify-between border-b border-slate-900/10 py-4 dark:border-slate-300/10 lg:mx-0 lg:border-0 lg:px-8">
            ...
            {breadcrumbs && breadcrumbs.length > 0 && (
              <Breadcrumb
                items={breadcrumbs}
                onToggleSidebar={menusideber}
                isMenusideber={isMenusideber}
              />
            )}
        ...
        <div
          className={`fixed fixed inset-0 inset-0 z-50 bg-black/20 backdrop-blur-sm dark:bg-slate-900/80 md:hidden lg:hidden ${
        ...
        >
          <div className="dark:highlight-white/5 fixed right-4 top-4 w-full max-w-xs rounded-lg bg-white p-6 text-base font-semibold text-slate-900 shadow-lg dark:bg-slate-800 dark:text-slate-400">
            ...
            <ul className="space-y-6">
              ...
              <li>
                <a
                  href="https://github.com/Saksorn21/ApiPlaygroundSwagger"
                  className="hover:text-sky-500 dark:hover:text-sky-400"
                >
                  GitHub
                </a>
        ...
        <footer className="mt-auto border-t border-gray-200 bg-white shadow-inner dark:border-gray-700 dark:bg-gray-800">
          <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <p className="text-center text-sm text-gray-500 dark:text-gray-400">
              &copy; {new Date().getFullYear()} API Platform. All rights
              reserved.
            </p>
          </div>
        </footer>
